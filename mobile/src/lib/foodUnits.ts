...
/**
 * 'unit' counts whole things: one red pepper, two eggs. It prints as "1 x", with
 * the same space as "80 g" and "1 tsp", so every quantity in the column has one
 * shape: a number, a space, a unit. The x reads as a count rather than a measure,
 * and stops a bare "1" looking like a weight that lost its unit.
 */
...
export function formatQuantity(quantity: number, unit: IngredientUnit): string {
  ...
  // Same number-space-unit shape as every other unit; the food name carries the noun.
