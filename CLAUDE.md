...
### Phase J — Local Cache + Offline Resilience (J1–J3b shipped; J3c not yet planned)
...
- [ ] **J3c — Graceful degradation for the genuinely-online-only surfaces.** The spec's §2 explicitly names a second category the J-series hasn't touched: features that **cannot** work offline and were only ever meant to "honestly say needs signal" (single inline state + retry, never a blank) — not be made to work without a network. J1/J2 built the `NeedsSignal` component and applied it to most of the spec's named list (`insights.tsx`, `weight.tsx`, `timeline.tsx`, `plan/[id].tsx`, `plans/browse.tsx`, `training.tsx`, and `food-search.tsx`'s live OFF search). Verified 2026-09-21 (grep against the current tree) that four of the spec's named surfaces still have **zero** offline-awareness — no `NeedsSignal`, no connectivity check, nothing: `app/(auth)/sign-in.tsx`, `app/(auth)/sign-up.tsx`, `app/(app)/subscription.tsx` (purchase/restore), `app/(app)/describe-meal.tsx` (Haiku). A fifth, `food-search.tsx`'s barcode lookup, degrades honestly today but via a modal `appAlert` rather than the spec's preferred inline pattern — a style inconsistency, not a blank-screen failure, lower priority than the four zero-coverage screens.
  - **Why this wasn't caught by J1–J3b's own final reviews:** those plans' scope was mutations and cache-first reads; a screen with no cache-relevant data (sign-in has nothing to cache) was never in a task's file list to review. This is a distinct kind of gap from everything J1–J3b fixed — genuinely different failure mode (missing UX for an unfixable case, not a data-loss/revert bug).
  - **How to apply:** brainstorm first to confirm the exact UX for each (a sign-in/sign-up submit while offline should probably disable the button + show inline text rather than a NeedsSignal card, which is built for a failed *read*, not a form *submit* — check `NeedsSignal`'s actual shape before assuming it fits all four screens unchanged). Likely a single small plan, not a multi-task SDD effort — four screens, all reads-or-writes into working infra (`useNetworkStore` already exists), no new outbox kind needed since these are correctly NOT going through the outbox (per spec's non-goals: "Offline insight regeneration or RevenueCat changes").
