...
### Phase J — Local Cache + Offline Resilience (J1–J3c all shipped)

Superseded by the offline-first design (card 284): `docs/superpowers/specs/2026-09-14-offline-first-design.md`. **The whole spec is now implemented and shipped.** J1 Foundation + Hardening (outbox, network store, sync pill), J2 Reads (cache-first dashboard/training/nutrition/recipes), J3a + J3b Writes (every mutation in the app goes through the outbox — check-in, food logging incl. drop/move sessions, favourites — with revert-only-on-dead-letter semantics throughout), J3c (the genuinely-online-only surfaces the spec's §2 says may honestly say "needs signal" rather than be made to work offline).

**J3c, completed 2026-09-23:**
- **Paywall purchase flow** (`app/(auth)/paywall.tsx`, commit `b1ecf70`) — the one confirmed real bug, not just missing polish: `getOfferings()` swallowed every failure into a bare `[]`, so a failed load hid the package list with no explanation and left the CTA live but silently dead. Fixed: `getOfferings()` now returns `{ packages, failed }`, the screen shows a `NeedsSignal` retry card on either failure (with different, honest copy for each), and the CTA is `disabled` whenever nothing is selected — a tap genuinely cannot fire.
- **Sign-in / sign-up** (`app/(auth)/sign-in.tsx`, `sign-up.tsx`, commit `fc75b8e`) — both already `appAlert`ed on any failure (never blank), but with whatever raw text supabase-js's client returns for a network failure. Fixed with a new `isAuthNetworkError` helper in `@/lib/supabase.ts` (a re-export of supabase-js's own `isAuthRetryableFetchError` — the real signal the client throws for a fetch failure, not a message-text guess): every auth-failure call site on both screens now shows on-brand "No signal" / "We couldn't reach the server. Check your connection and try again." for a genuine network failure, leaving every other auth error (wrong password, unconfirmed email, etc.) untouched. Apple's own native sign-in dialog failures are a separate error surface, deliberately not touched.
- **Re-investigated and found to be non-issues** (an earlier scoping pass here, based on a keyword grep rather than reading the code, wrongly flagged these): `describe-meal.tsx` already has honest, well-written `InlineError` handling including a good offline-specific fallback message. `subscription.tsx`'s status display already degrades gracefully via J2's cached store; its one gap (a silently-null `managementURL` on a failed load) already has a harmless fallback URL.
- **Found but NOT fixed, still open for whoever picks it up:** `mobile/__tests__/components/CopyMealFromDayModal.test.tsx` (from J3b) hardcodes a literal date string (`'2026-09-19'`) expecting a component to render it as "YESTERDAY" — breaks every day as real time moves past the date it was written on (confirmed broken as of 2026-09-23). An attempted live-date fix broke the test further in a way not fully diagnosed; reverted rather than ship a half-understood change. Not blocking anything, but will keep silently failing `npm test` as time passes.

**Device verification (cards 253/258) is the one remaining open item from the whole offline-first effort** — no recorded pass/fail on a real device since build 15. Check with Emma before cutting the next build.
