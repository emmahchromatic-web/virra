...
### Phase J — Local Cache + Offline Resilience (J1–J3b shipped; J3c in progress)

Superseded by the offline-first design (card 284): `docs/superpowers/specs/2026-09-14-offline-first-design.md`. Shipped: J1 Foundation + Hardening (outbox, network store, sync pill), J2 Reads (cache-first dashboard/training/nutrition/recipes), J3a + J3b Writes (every mutation in the app now goes through the outbox — check-in, food logging incl. drop/move sessions, favourites — with revert-only-on-dead-letter semantics throughout).

**J3c — Graceful degradation for the genuinely-online-only surfaces.** The spec's §2 names a second category the earlier J-series never touched: features that **cannot** work offline and were only ever meant to "honestly say needs signal" (single inline state + retry, never a blank) — not be made to work without a network.

- ✅ **Paywall purchase flow** (`app/(auth)/paywall.tsx`, commit `b1ecf70`) — the one confirmed real bug, not just missing polish: `getOfferings()` swallowed every failure into a bare `[]`, so a failed load hid the package list with no explanation and left the CTA live but silently dead (tapping it did nothing, since no package was ever selected). Fixed: `getOfferings()` now returns `{ packages, failed }` so a genuine failure is distinguishable from a legitimate empty response; the screen shows a `NeedsSignal` card with a retry in place of the package list on either failure (different copy for each — never "check your connection" when the call actually succeeded and just came back empty); the CTA is `disabled` whenever there's no selected package, so a tap genuinely cannot fire; the pre-existing silent `if (!selected) return` in `handlePurchase` is now a proper `InlineError` message too, as defense in depth.
- [ ] `app/(auth)/sign-in.tsx` / `sign-up.tsx` — already `appAlert` on any failure (never blank), but the message is whatever raw string Supabase's client returns for a network failure, not app-voice copy. Real gap is modest (copy quality, not a dead-end), not yet started.
- **Re-investigated and found to be non-issues** (an earlier pass here, based on a keyword grep rather than reading the code, wrongly flagged these): `describe-meal.tsx` already has honest, well-written `InlineError` handling including a good offline-specific fallback message — no work needed. `subscription.tsx`'s status display already degrades gracefully via J2's cached store; its only gap (a silently-null `managementURL` on a failed load) already has a harmless fallback URL — very low priority, probably not worth touching.
- **Found but NOT fixed, flagged for whoever picks this up next:** `mobile/__tests__/components/CopyMealFromDayModal.test.tsx` (from J3b) hardcodes a literal date string (`'2026-09-19'`) expecting a component to render it as "YESTERDAY" — this breaks every day as real time moves past the date it was written on (confirmed broken as of 2026-09-23, 4 days after J3b shipped it). An attempted fix (computing the date live relative to `new Date()`) unexpectedly broke the test further in a way not fully diagnosed before this session ended — reverted rather than leaving a partially-understood change in place. Needs someone to sit with it properly; not blocking anything, but it will keep silently failing `npm test` as time passes until it's fixed.
  - **How to apply the remaining scope:** brainstorm first to confirm the exact UX for sign-in/sign-up (a form submit while offline should probably disable the button + show inline text rather than a `NeedsSignal` card, which is built for a failed *read*, not a form *submit*). Small, bounded work — no new outbox kind needed (per spec's non-goals: "Offline insight regeneration or RevenueCat changes").
